...
const genrelist = async () => {
    let htmlll = ""
    const res = await fetch("https://api.themoviedb.org/3/genre/movie/list?api_key=6b2dec73b6697866a50cdaef60ccffcb")
...
const firstpage = async () => {
    const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=6b2dec73b6697866a50cdaef60ccffcb&sort_by=popularity.desc&include_adult=false&page=${intialPage}&with_genres=${categoryId}`);
    const data = await res.json();
...
const airingTodayfun = async () => {
    const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=6b2dec73b6697866a50cdaef60ccffcb&sort_by=popularity.desc&include_adult=false&with_genres=${categoryId}`);
    const data = await res.json();
...
const searchfun = (movie) => {
    ...
    <a class="posterlink" href="${url}"> <img class="poster" data-id="${movie.id
         ...
         <div class="date_rating tvshows_date_rating">
             <p class="date date_search">${dateFormatter(movie.release_date)}</p><span class="dot dot2 recommendTvShow_date_dot"></span>
